...
def index(request):
    if request.session.has_key('is_logged'):
        ...
        x= user.userprofile.Savings+addmoney_info.sum1 - addmoney_info.sum
        y= user.userprofile.Savings+addmoney_info.sum1 - addmoney_info.sum
        if x<0:
    ...
    #if request.session.has_key('is_logged'):
        return render(request,'home/index.html', {'page_obj' : page_obj,'addmoney':addmoney_info})
